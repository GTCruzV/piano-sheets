...
"publicaciones" estilo Instagram: cualquier usuario puede subir tantas
partituras/proyectos como quiera (con nombre y foto de referencia
cuadrada), separados en dos apartados independientes ("sheet" =
Partituras, "controller" = Controladores). Cada publicación queda
"pendiente" hasta que el admin (tirhiv) la aprueba o la rechaza (el
admin se autoaprueba). Solo las aprobadas se ven en la tienda pública,
y cualquier usuario logueado les puede dar like.
...
def init_db():
    ...
    # --- migración: "kind" separa Partituras de Controladores. Antes
    # vivían todas mezcladas en la misma tabla sin distinción; lo que
    # ya existía se marca como 'sheet' para no perderlo.
    if not _column_exists(conn, "sheets", "kind"):
        conn.execute("ALTER TABLE sheets ADD COLUMN kind TEXT NOT NULL DEFAULT 'sheet'")
...
def validate_controller(text):
    """Validación (ligera) del lenguaje del Editor de teclado. El
    cliente ya valida a fondo esta sintaxis antes de mandarla (ve
    validate_controller en piano_autoplayer.py); aquí solo evitamos
    contenido vacío o absurdamente largo, sin reimplementar todas las
    reglas del lenguaje."""
    text = text or ""
    if not text.strip():
        return False, "El controlador está vacío."
    if len(text) > 50000:
        return False, "El controlador es demasiado largo."
    return True, None


VALID_KINDS = ("sheet", "controller")


def normalize_kind(value):
    value = (value or "sheet").strip().lower()
    return value if value in VALID_KINDS else "sheet"
...
def sheet_public(row, db=None, current_user_id=None):
    likes_count = 0
    liked_by_me = False
    if db is not None:
        likes_count = db.execute(
            "SELECT COUNT(*) AS c FROM likes WHERE sheet_id = ?", (row["id"],)
        ).fetchone()["c"]
        if current_user_id:
            liked_by_me = db.execute(
                "SELECT 1 FROM likes WHERE sheet_id = ? AND user_id = ?",
...
SHEET_JOIN_SQL = """
    SELECT sheets.id, sheets.name, sheets.created_at, sheets.status,
           sheets.reject_reason, sheets.user_id, sheets.kind,
...
def list_sheets():
    ...
    user = current_user()
    rows = db.execute(
        SHEET_JOIN_SQL + " WHERE sheets.status = 'approved' AND sheets.kind = ? ORDER BY sheets.id DESC",
        (kind,),
    ).fetchall()
    return jsonify([sheet_public(r, db, user["id"] if user else None) for r in rows])
...
def user_profile(username):
    ...
    Se puede filtrar por ?kind=sheet|controller; si no se manda,
    devuelve de ambos apartados."""
    ...
    if kind:
        rows = db.execute(
            SHEET_JOIN_SQL + " WHERE sheets.user_id = ? AND sheets.status = 'approved' AND sheets.kind = ? ORDER BY sheets.id DESC",
            (user_row["id"], normalize_kind(kind)),
        ).fetchall()
    else:
        rows = db.execute(
    ...
    return jsonify({
        ...
        "items": [sheet_public(r, db, user["id"] if user else None) for r in rows],
    })


@app.route("/api/sheets/mine", methods=["GET"])
def my_sheets():
    """Devuelve TODAS las publicaciones (pendientes/aprobadas/
    rechazadas) del usuario logueado en el apartado indicado por
    ?kind=. No hay límite de publicaciones por usuario: esto es solo
    para que la app le muestre el estado de revisión de cada una."""
    user, err = require_user()
    if err:
        return err
    kind = normalize_kind(request.args.get("kind"))
    db = get_db()
    rows = db.execute(
        SHEET_JOIN_SQL + " WHERE sheets.user_id = ? AND sheets.kind = ? ORDER BY sheets.id DESC",
        (user["id"], kind),
    ).fetchall()
    items = [sheet_public(r, db, user["id"]) for r in rows]
    return jsonify({"items": items, "item": items[0] if items else None})


@app.route("/api/sheets", methods=["POST"])
def upload_sheet():
    """Crea una publicación nueva. No hay límite: un usuario puede
    tener tantas publicaciones (pendientes o aprobadas) como quiera,
    tanto en Partituras como en Controladores."""
    user, err = require_user()
    if err:
        return err
    ...
    if kind == "controller":
        ok, err_msg = validate_controller(content)
        err_prefix = "Controlador inválido: "
    else:
        ok, err_msg = validate_sheet(content)
        err_prefix = "Partitura inválida: "
    if not ok:
        return jsonify({"error": err_prefix + err_msg}), 400
    ...
    cur = db.execute(
        "INSERT INTO sheets (name, content, user_id, created_at, status, photo_file, reviewed_at, kind) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
        (name, content, user["id"], now, status, photo_file, now if is_admin else None, kind),
    )
    db.commit()
...
def delete_sheet(sheet_id):
    ...
    db.execute("DELETE FROM likes WHERE sheet_id = ?", (sheet_id,))
    db.execute("DELETE FROM sheets WHERE id = ?", (sheet_id,))
    db.commit()
    return jsonify({"ok": True})


@app.route("/api/sheets/<int:sheet_id>/like", methods=["POST"])
def like_sheet(sheet_id):
    """Da o quita el like del usuario logueado a una publicación
    aprobada (toggle: si ya le había dado like, se lo quita)."""
    user, err = require_user()
    if err:
        return err
    db = get_db()
    row = db.execute("SELECT id, status FROM sheets WHERE id = ?", (sheet_id,)).fetchone()
    if not row:
        return jsonify({"error": "Esa publicación ya no existe."}), 404
    if row["status"] != "approved":
        return jsonify({"error": "Esa publicación todavía no está disponible."}), 403

    already = db.execute(
        "SELECT 1 FROM likes WHERE sheet_id = ? AND user_id = ?", (sheet_id, user["id"])
    ).fetchone()
    if already:
        db.execute("DELETE FROM likes WHERE sheet_id = ? AND user_id = ?", (sheet_id, user["id"]))
        liked = False
    else:
        db.execute(
            "INSERT INTO likes (sheet_id, user_id, created_at) VALUES (?, ?, ?)",
            (sheet_id, user["id"], datetime.utcnow().isoformat()),
        )
        liked = True
    db.commit()
    count = db.execute("SELECT COUNT(*) AS c FROM likes WHERE sheet_id = ?", (sheet_id,)).fetchone()["c"]
...
def pending_sheets():
    ...
    db = get_db()
    rows = db.execute(
        SHEET_JOIN_SQL + " WHERE sheets.status = 'pending' AND sheets.kind = ? ORDER BY sheets.id ASC",
        (kind,),
    ).fetchall()
    items = []
    for r in rows:
        content_row = db.execute("SELECT content FROM sheets WHERE id = ?", (r["id"],)).fetchone()
        item = sheet_public(r, db, user["id"])
...
def admin_delete_user(username):
    """Borra una cuenta por completo (el admin no puede borrarse a sí
    mismo): sus tokens, su publicación y el registro de usuario. El
    nombre de usuario queda libre para que alguien se registre de
    nuevo con él."""
    admin_user, err = require_admin()
    if err:
        return err
    ...
    if not target:
        return jsonify({"error": "Ese usuario no existe."}), 404
    if target["is_admin"]:
        return jsonify({"error": "No puedes borrar al admin."}), 400

    # borra las publicaciones (y sus fotos) y las fotos de perfil del
    # usuario del disco; si algún archivo ya no existe, no pasa nada
    sheet_rows = db.execute(
        "SELECT photo_file FROM sheets WHERE user_id = ?", (target["id"],)
    ).fetchall()
    for r in sheet_rows:
        if r["photo_file"]:
            try:
                os.remove(os.path.join(PHOTOS_DIR, r["photo_file"]))
            except OSError:
                pass
    if target["photo_file"]:
        try:
            os.remove(os.path.join(PHOTOS_DIR, target["photo_file"]))
        except OSError:
            pass

    sheet_ids = [r["id"] for r in db.execute("SELECT id FROM sheets WHERE user_id = ?", (target["id"],))]
    for sid in sheet_ids:
        db.execute("DELETE FROM likes WHERE sheet_id = ?", (sid,))
    db.execute("DELETE FROM likes WHERE user_id = ?", (target["id"],))
    db.execute("DELETE FROM tokens WHERE user_id = ?", (target["id"],))
    db.execute("DELETE FROM sheets WHERE user_id = ?", (target["id"],))
