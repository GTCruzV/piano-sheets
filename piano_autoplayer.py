...
class Api:
    ...
    def tienda_drop_post_photo(self, data_url, filename="foto.png"):
        """Igual que tienda_pick_post_photo(), pero para cuando la
        imagen se soltó arrastrándola (drag & drop) en vez de elegirla
        con el explorador: recibe la imagen ya leída en el navegador
        como data URL (base64) y la guarda en un archivo temporal para
        poder subirla igual que si se hubiera elegido con el botón."""
        try:
            header, b64data = data_url.split(",", 1)
        except (ValueError, AttributeError):
            return None
        ext = os.path.splitext(self._sanitize_filename(filename or "foto.png"))[1].lower()
        ...
        try:
            ...
            return None
        tmp_dir = os.path.join(BASE_DIR, "_tmp_drop")
        os.makedirs(tmp_dir, exist_ok=True)
        path = os.path.join(tmp_dir, f"post_photo_{uuid.uuid4().hex}{ext}")
        try:
            ...
            return None
        self._pending_post_photo = path
        return os.path.basename(filename or path)
    ...
    def _tienda_mine_impl(self, kind="sheet"):
        """Devuelve TODAS las publicaciones propias (pendientes,
        aprobadas o rechazadas) en el apartado indicado. Ya no hay
        límite de una sola publicación por usuario: esto es solo para
        que la app muestre el estado de revisión de cada una."""
        if not self.session:
            return {"ok": False, "error": "Inicia sesión primero.", "items": [], "item": None}
        try:
            resp = requests.get(
            ...
            data = resp.json()
            if resp.status_code != 200:
                return {"ok": False, "error": data.get("error", "No se pudo consultar."), "items": [], "item": None}
            items = data.get("items") or []
            return {"ok": True, "items": items, "item": data.get("item")}
        except Exception as e:
            return {"ok": False, "error": f"No se pudo conectar con la tienda: {e}", "items": [], "item": None}

    def tienda_like(self, req_id, sheet_id):
        return self._async(req_id, self._tienda_like_impl, sheet_id)

    def _tienda_like_impl(self, sheet_id):
        """Da o quita el like a una publicación (toggle)."""
        if not self.session:
            return {"ok": False, "error": "Inicia sesión primero."}
        try:
            resp = requests.post(
                f"{TIENDA_API_URL}/sheets/{sheet_id}/like",
                headers=self._auth_headers(),
                timeout=15,
            )
            data = resp.json()
            if resp.status_code != 200:
                return {"ok": False, "error": data.get("error", "No se pudo dar like.")}
            return {"ok": True, "liked": data.get("liked"), "likes_count": data.get("likes_count")}
        except Exception as e:
            return {"ok": False, "error": f"No se pudo conectar con la tienda: {e}"}
